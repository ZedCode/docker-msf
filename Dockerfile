...
RUN cd /home/mallory && gpg --keyserver hkp://keys.gnupg.net --recv-keys D39DC0E3 && curl -o rvm.sh -L get.rvm.io && cat rvm.sh | bash -s stable --autolibs=enabled --ruby=2.1.5 && echo "source /home/mallory/.rvm/scripts/rvm" >> /home/mallory/.bash_profile
...
# An Ubuntu hack to preserve the PATH environment variable (with all our ruby binaries) when using sudo
RUN echo "alias sudo='sudo env PATH=\$PATH'" >> /home/mallory/.bashrc
USER root
